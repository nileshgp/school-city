...
@Component({
  selector: 'app-englishtest2',
  templateUrl: './englishtest2.component.html',
  styleUrls: ['./englishtest2.component.css']
})

export class Englishtest2Component implements OnInit, OnDestroy {
  tests: Test[];
      ...
      .subscribe(
        (tests: Test[]) => {
          ...
          for (i = 0; i < (tests).length; i++) {
            eTests.push([tests[i].englishTest2]);
            studentScoresArray.push({name: tests[i].student, score: Number(tests[i].englishTest2)});
          }
